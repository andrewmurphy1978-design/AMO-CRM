...
import EventViewDialog, {
  type EventViewDialogLabels,
} from "./calendar-app/event-view-dialog";
import EventDialog, {
  type EventDialogLabels,
  type EventDialogTarget,
} from "./calendar-app/event-dialog";
...
function DayColumn({
  ...
  const timed: TimedEvent<CalendarEventSummary>[] = events
    ...
    .map((e) => {
      ...
      return {
        ...
        endMin: Math.min(
          gridEnd,
          Math.max(
            minutesSinceGridStart(end),
            minutesSinceGridStart(start) + 15,
          ),
        ),
      };
    })
    .filter(
      (e) =>
        e.endMin > 0 && e.startMin < (GRID_END_HOUR - GRID_START_HOUR) * 60,
    );
  ...
  return (
    <div
      onClick={handleClick}
      className="relative min-w-0 flex-1 cursor-pointer border-l border-card-border first:border-l-0"
    >
      {hourMarks.map((h) => (
        <div
          key={h}
      ...
      {positioned.map(({ event, segments }) =>
        segments.map((seg, i) => {
          ...
          const natural =
            ((seg.endMin - seg.startMin) / 60) * ROW_HEIGHT - (isLast ? 1 : 0);
          // Only the first/last segment need room for the title — see the
          // same pattern in day-grid-view.tsx.
          const height =
            isFirst || isLast ? Math.max(MIN_BLOCK_HEIGHT, natural) : natural;
            ...
            >
              {isFirst && (
                <>
                  ...
                  {link &&
                    (link.contactName || link.projectName || link.taskName) && (
      ...
      {events.length === 0 && (
        <div className="absolute inset-0 flex items-start justify-center pt-6 text-xs text-soft">
          {labels.noEvents}
        </div>
      )}
    </div>
...
function ThreeDayGrid({
  ...
  const allDayByDay = eventsByDay.map((events) =>
  ...
  useEffect(() => {
    ...
    setAllDayHeight(
      allDayRef.current ? allDayRef.current.getBoundingClientRect().height : 0,
    );
  ...
  return (
    ...
    <div className="mt-1.5 sm:mt-3 overflow-hidden rounded-xl border border-card-border">
      <div
        ...
        style={{
          maxHeight: VISIBLE_HOURS * ROW_HEIGHT + headerHeight + allDayHeight,
        }}
      >
        <div
          ref={headerRef}
          className="sticky top-0 z-20 flex border-b border-card-border bg-field-bg"
        >
          <div className="w-10 shrink-0" />
          {days.map((day, i) => (
            <div
              key={i}
              className="min-w-0 flex-1 border-l border-card-border py-1.5 text-center first:border-l-0"
            >
              ...
              <p
                className={
                  isToday(day)
                    ? "text-sm font-bold text-amo-lime"
        ...
        {hasAllDay && (
          <div
            ref={allDayRef}
            className="sticky z-20 flex border-b border-card-border bg-card-bg"
        ...
        <tbody>
          {days.map((day, i) => (
            <tr
              ...
              className="cursor-pointer border-b border-card-border last:border-b-0 hover:bg-black/5"
            >
...
export default function CalendarCard({
  ...
  const [dialogTarget, setDialogTarget] = useState<EventDialogTarget | null>(
    null,
  );
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-card-border bg-card-bg p-2 shadow-sm sm:p-5">
      ...
      {/* No "Open Calendar" button — clicking anywhere on the card that
          ...
          `display:contents` keeps this purely an event boundary. The grid
          and table below already have plenty of their own click handlers
          (click empty space to create an event, click an event to view
          it) — the single stopPropagation on their wrapper stops any of
          those clicks from *also* bubbling up into this "go to
          /calendar-app" handler, while each one's own handler still fires
          normally first. The two event dialogs and the toast are
          deliberately this div's *siblings* (outside it) for the same
          reason as the Email card's own dialogs. */}
      <div
        role="button"
        tabIndex={connected ? 0 : -1}
        onClick={() => connected && router.push("/calendar-app")}
        onKeyDown={(e) => {
      ...
      {toast && (
        <div className="fixed inset-x-0 top-4 z-[60] flex justify-center px-4">
          <div className="rounded-lg bg-emerald-600 px-6 py-3 text-sm font-semibold text-white shadow-lg">
            {toast}
          </div>
        </div>
      )}
