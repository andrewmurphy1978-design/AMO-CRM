...
function EmailRow({
  ...
  return (
    <li className={clsx("overflow-hidden transition-colors hover:bg-black/5", highlight ? "bg-amo-gold/20" : index % 2 === 1 ? "bg-black/[0.03]" : "")}>
      {/* Desktop/tablet (sm and up): the full icon-and-column layout,
          unchanged. Hidden below `sm` in favor of the simplified mobile
          row right below — the fixed-width columns and icon cluster don't
          have room to fit on a narrow screen. */}
      <div className={clsx("hidden items-center gap-2 py-1.5 pl-3 pr-4 sm:grid", ROW_GRID_COLUMNS)}>
      ...
      {/* Mobile only (below `sm`): name on top, subject right below it,
