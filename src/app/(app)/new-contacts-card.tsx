  ...
  return (
    ...
    >
      ...
      <div className="mt-1 flex items-center justify-between gap-2">
        <span
          className={`shrink-0 rounded-full px-2 py-1 text-xs font-medium ${STAGE_COLORS[contact.stage]}`}
        >
  ...
  return (
    <section className="shrink-0 overflow-hidden rounded-xl border border-card-border">
      <div className={`flex items-center gap-2 px-3 py-2 ${colors.headerBg}`}>
        <h3
          className={`text-xs font-semibold uppercase tracking-wide ${colors.headerText}`}
        >
          {label}
        </h3>
        <span
          className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${colors.badgeBg} ${colors.badgeText}`}
        >
          {contacts.length}
        </span>
      </div>
      <ul
        className="divide-y divide-card-border overflow-y-auto bg-card-bg"
        style={{ maxHeight }}
        ...
        {contacts.map((contact, i) => (
          <li key={contact.id}>
            <ContactRow
              ...
              bg={i % 2 === 0 ? LIGHT_ROW_BG : ALT_ROW_BG}
            />
          </li>
        ))}
      </ul>
  ...
  return (
    <div className="relative flex flex-col overflow-hidden rounded-2xl border border-card-border bg-card-bg p-2 shadow-sm sm:p-5">
      ...
      {/* No "Open Contacts" button — clicking anywhere on the card that
          ...
          own <ul> stops propagation so a row click still just opens that
          contact instead of also navigating to /contacts). */}
      <div
        role="button"
        tabIndex={0}
        onClick={() => router.push("/contacts")}
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") router.push("/contacts");
        }}
