    ...
    <div className="flex items-center justify-between gap-2 py-1.5">
      <div className="flex items-center gap-1.5">
        ...
        <span
          className={clsx(
            "text-xs",
            awayIsJays ? BLUE_JAYS_ACCENT : "text-ink/70",
          )}
        >
        ...
        <span
          className={clsx(
            "text-xs",
            homeIsJays ? BLUE_JAYS_ACCENT : "text-ink/70",
          )}
        >
...
export default function SportsCard({
  ...
  const hasMlb = Boolean(
    snapshot?.mlb.seriesGames.length ||
          ...
          {hasMlb && snapshot && (
            <div className="py-2 last:pb-0">
              <div className="flex items-center gap-1.5">
                {/* eslint-disable-next-line @next/next/no-img-element -- external team-logo CDN, not a local asset */}
                <img
                  src={snapshot.mlb.teamLogo}
                  alt={snapshot.mlb.teamName}
                  className="h-5 w-5 object-contain"
                />
                <p className="text-[10px] font-semibold uppercase tracking-wide text-soft">
                  {snapshot.mlb.inPostseason
                    ? labels.series
                    : snapshot.mlb.teamName}
                </p>
              </div>
