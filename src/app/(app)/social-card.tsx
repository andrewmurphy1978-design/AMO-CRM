...
function StatCell({
  ...
  snapshot: SocialSnapshotView | undefined;
  labels: SocialLabels;
  soft: string;
}) {
  ...
  if (snapshot.views !== null)
    lines.push({ key: "views", label: labels.views, value: snapshot.views });
  for (const stat of snapshot.extraStats) {
    ...
    lines.push({
      key: stat.key,
      label: labels.statLabels[stat.key],
      value: stat.value,
    });
  }
    ...
    <div className="space-y-0.5 text-xs leading-tight">
      {lines.map((line) => (
        <div key={line.key} className="flex items-baseline">
          <span className={clsx(LABEL_WIDTH, "shrink-0", soft)}>
            {line.label}
          </span>
          ...
          <span className="ml-5 w-10 shrink-0 text-right font-medium tabular-nums">
            {fmt(line.value)}
          </span>
        </div>
      ))}
...
export default function SocialCard({
  ...
  const byKey = new Map(
    snapshots.map((s) => [`${s.platform}|${s.language}`, s]),
  );
